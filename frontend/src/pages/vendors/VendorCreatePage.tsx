...
export function VendorCreatePage() {
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      message.success(t('vendors.createSuccess'));
      navigate('/vendors');
    } catch (err: any) {
      message.error(err.response?.data?.error?.message || t('vendors.createFailed'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <Title level={3}>{t('vendors.createVendor')}</Title>
        ...
        <Form layout="vertical" onFinish={handleSubmit} initialValues={{ payment_terms: 30, credit_limit: 0 }}>
          ...
          <Form.Item>
            <Space>
              ...
              <Button onClick={() => navigate('/vendors')}>{t('common.cancel')}</Button>
            </Space>
          </Form.Item>
        </Form>
      </Card>

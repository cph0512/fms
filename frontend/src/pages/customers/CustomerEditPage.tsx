...
export function CustomerEditPage() {
  ...
  useEffect(() => {
    ...
    customersApi
      ...
      .catch(() => message.error(t('customers.loadFailed')))
  ...
  const handleSubmit = async (values: any) => {
    ...
    try {
      ...
      message.error(err.response?.data?.error?.message || t('customers.updateFailed'));
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div>
      <Title level={3}>{t('customers.editCustomer')} - {customerCode}</Title>
